...
const DEFAULT_SKILLS_PREFIXES = ["/vault/skills"] as const;

export interface MentionHrefParseOptions {
  skillsPrefixes?: readonly string[];
}
...
function normalizeSkillsPrefixes(prefixes?: readonly string[]) {
  const normalized = (prefixes ?? DEFAULT_SKILLS_PREFIXES)
  ...
  return normalized.length > 0 ? normalized : [...DEFAULT_SKILLS_PREFIXES];
}

function isSupportedSkillsPath(path: string, options?: MentionHrefParseOptions) {
  const skillsPrefixes = normalizeSkillsPrefixes(options?.skillsPrefixes);
  return skillsPrefixes.some((prefix) => path.startsWith(`${prefix}/`));
}
...
export function getInternalSkillsHref(
  rawHref: string,
  options?: MentionHrefParseOptions,
): string | null {
  const parsed = parseHref(rawHref);
  if (!parsed || parsed.isAbsoluteHttp) return null;
  if (!isSupportedSkillsPath(parsed.path, options)) return null;
...
export function parseMentionHref(
  ...
  return parsePersistedMentionQuery(mention);
}
...
export function storageMarkdownToEditorMarkdown(options: {
  ...
  const { originalMarkdown, renderedMarkdown, skillsPrefixes } = options;
  ...
  if (renderedMarkdown) {
    ...
    while ((linkMatch = MARKDOWN_LINK_REGEX.exec(renderedMarkdown)) !== null) {
      ...
      const mention = parseMentionHref(href, { skillsPrefixes });
...
export function editorMarkdownToStorageMarkdown(
  ...
  const linkedMentions = editorMarkdown.replace(
    ...
    (match, _label: string, href: string) => {
      const mention = parseMentionHref(href, options);
