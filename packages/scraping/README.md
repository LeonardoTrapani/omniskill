...
Pipeline deterministica per importare skill dal formato pubblico (skills.sh) nel database better-skills, risolvere tutti i cross-reference tra skill e tra skill-risorse, e generare il grafo di link (`skill_link`).

## Panoramica

Il problema: le skill provengono da una directory `parsed-skills/` con file markdown e risorse locali. Il markdown contiene centinaia di riferimenti testuali (path relativi, URL, slug) che devono essere convertiti in token UUID-based (`[[skill:<uuid>]]`, `[[resource:<uuid>]]`) per il sistema di mentions di better-skills.
