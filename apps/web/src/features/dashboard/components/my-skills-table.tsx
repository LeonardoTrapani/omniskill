      ...
      <div className="min-h-0 flex-1 overflow-y-auto">
        ...
        {!isLoading &&
          ...
          skills.map((skill, index) => (
            <div
              key={skill.id}
              className="group grid grid-cols-[40px_1fr_56px] items-center border-t border-border px-6 py-4 transition-colors hover:bg-secondary/50"
            >
              <span className="text-sm text-muted-foreground tabular-nums">{index + 1}</span>

              <Link href={buildSkillHref(skill.id)} className="min-w-0 pr-4">
              ...
              <div className="flex justify-end">
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    ...
                    <DropdownMenuItem onClick={() => router.push(buildSkillEditHref(skill.id))}>
